  ...
  return <header className="sticky top-0 z-50 border-b border-black/5 bg-white/95 backdrop-blur">
    <div className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 lg:px-8">
      <Link to="/" className="flex items-center gap-3" aria-label="Duncan Village Drop-in Centre home">
        <span className="grid h-11 w-11 place-items-center rounded-full bg-[#1B5E20] text-white"><HeartHandshake /></span>
    ...
    {open && <nav className="border-t bg-white px-5 py-5 xl:hidden" aria-label="Mobile navigation">{nav.map(([label,path]) => <NavLink onClick={() => setOpen(false)} className="block border-b py-3 text-sm font-bold" key={path} to={path}>{label}</NavLink>)}<Link onClick={() => setOpen(false)} to="/support" className="mt-4 block rounded-full bg-[#E65100] px-5 py-4 text-center text-sm font-bold text-white">GET SUPPORT</Link></nav>}
  </header>;
}